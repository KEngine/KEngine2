...
class GameUniformBuffer: NSObject {
    ...
    init(data:[Float],scene:GameViewController) {
        super.init()
        m_data = data
        m_scene = scene
    ...
    func copyDataToBuffer(){
        memcpy(self[m_scene.m_render.m_bufferIndex].contents(), m_data, sizeofValue(m_data[0]) * m_data.count)
    }
    ...
    func buffer()->MTLBuffer{
        copyDataToBuffer()
        return self[m_scene.m_render.m_bufferIndex]
    }
